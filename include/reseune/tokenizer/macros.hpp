...
#define my                    &t::
#define NEXT                  ((cursor)++)
#define NO_MATCH              (span<LABEL_T>{nullptr, nullptr, false})
#define NUL_HERE              (0 == HERE)
#define POS                   (cursor.pos())
#define REWIND                (cursor.set_pos(start))
#define SPAN                  span<LABEL_T>{start, POS}
#define START                 log("%s: Enter.", __FUNCTION__); MARK(start); span<LABEL_T> match{NO_MATCH};
...
#define RETURN_EMPTY          {log("%s: Returning empty after moving %zu.", __FUNCTION__, POS - start); return span<LABEL_T>{POS, POS};}
...
#define MATCH_F(name, ...)    constexpr inline span<LABEL_T> name(__VA_ARGS__)
